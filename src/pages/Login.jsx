...
const SuccessAnimation = () => (
  <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="text-center py-8">
    ...
    <motion.h3 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-2xl font-semibold text-white mb-2">
      Welcome back!
    </motion.h3>
    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="text-white/80">
      Redirecting to your dashboard...
    </motion.p>
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="flex justify-center gap-1 mt-6">
      {[0, 1, 2].map((index) => (
        <motion.div key={index} className="w-2 h-2 bg-white rounded-full" animate={{ scale: [1, 1.5, 1], opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.5, repeat: Infinity, delay: index * 0.2 }} />
      ))}
    </motion.div>
  </motion.div>
);
...
const ErrorAnimation = ({ error, onRetry }) => (
  <motion.div key="error" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="text-center py-8">
    <motion.div initial={{ scale: 0, rotate: 180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 200, damping: 15 }} className="w-20 h-20 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6 backdrop-blur-sm">
      <XCircle className="text-white" size={40} />
    </motion.div>
    <motion.h3 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-2xl font-semibold text-white mb-2">
      Login Failed
    </motion.h3>
    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-white/80 mb-6">
      {error || 'Invalid email or password. Please try again.'}
    </motion.p>
    <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} onClick={onRetry} className="px-8 py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl font-medium transition-all duration-300 border border-white/30 backdrop-blur-sm" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      Try Again
    </motion.button>
  </motion.div>
);
...
export default function Login() {
  ...
  const [isForgotModalOpen, setIsForgotModalOpen] = useState(false); // Forgot password modal state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        if (!name || !email || !password) { setError('Please fill out all fields.'); setIsLoading(false); return; }
  ...
  return (
    <>
      <style>{`
        ...
        .waves { position: absolute; bottom: 0; left: 0; width: 100%; height: 30vh; min-height: 200px; max-height: 300px; }
        ...
        .particle { position: absolute; background: rgba(255, 255, 255, 0.1); border-radius: 50%; animation: float 15s infinite linear; }
        .dark .particle { background: rgba(199, 210, 254, 0.1); }
      ...
      <div className="flex flex-col min-h-screen login-bg dark:bg-gray-950 px-6 py-8 sm:px-12 relative">
        ...
        <div className="particles">
          {[...Array(15)].map((_, i) => (
            <div key={i} className="particle" style={{ left: `${Math.random() * 100}%`, width: `${Math.random() * 4 + 2}px`, height: `${Math.random() * 4 + 2}px`, animationDelay: `${Math.random() * 15}s`, animationDuration: `${Math.random() * 10 + 15}s`, }} />
          ))}
        </div>
                    ...
                    {isLogin && (
                      <div className="text-right">
                        <button type="button" onClick={() => setIsForgotModalOpen(true)} 
                          ...
                          className="text-sm font-medium text-gray-700 dark:text-white/80 hover:text-black dark:hover:text-white transition-colors duration-300">
